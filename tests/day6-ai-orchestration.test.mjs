...
test("official offices use the evidence-backed curated Layanan reader", () => { assert.match(tools, /layanan_public_offices/); assert.match(tools, /source_name,evidence_url/); assert.doesNotMatch(tools, /from\("representative_offices"\)/); });
test("official contacts use the parent-safe curated Layanan reader", () => { assert.match(tools, /verified_official_contacts/); assert.match(tools, /layanan_public_contact_channels/); assert.match(tools, /source_name,evidence_url/); assert.doesNotMatch(tools, /from\("office_contact_channels"\)/); });
test("official news tool uses only the curated News V2 reader", () => { assert.match(tools, /verified_official_news/); assert.match(tools, /from\("news_public_items"\)/); assert.doesNotMatch(tools, /from\("news_items"\)/); });
